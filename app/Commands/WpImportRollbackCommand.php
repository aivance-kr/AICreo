 ...
 * 게시판(boards)·게시판 카테고리(board_categories)도 워드프레스 메뉴 이관으로
 * 새로 만들어진 것(wp_term_id NOT NULL)이라 함께 삭제 대상이다.
...
class WpImportRollbackCommand extends BaseCommand
{
    ...
    public function run(array $params)
    {
        if (! CLI::getOption('force')) {
            $confirmed = CLI::prompt('이관된 게시판·카테고리·글·페이지·댓글·미디어를 전부 삭제합니다. 계속할까요?', ['y', 'n']);
            if ($confirmed !== 'y') {
        ...
        CLI::write('롤백 완료. redirects 테이블(SEO 보존 목적)은 그대로 둡니다.', 'green');
    }
}
