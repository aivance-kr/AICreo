 ...
 *
 * 게시판/카테고리 구조는 워드프레스 메뉴에서 뽑는다 — 관리자가 미리 게시판을
 * 만들어둘 필요 없음. --dry-run 으로 사용 가능한 메뉴 목록을 먼저 확인할 것.
 */
class WpImportCommand extends BaseCommand
{
    ...
    protected $usage       = 'wp:import <path-to-export.xml> --menu <메뉴 slug> --uploads-dir <wp-content/uploads 경로> [--dry-run]';
    protected $arguments   = [
        'path' => 'WXR(.xml) 파일 경로',
    ];
    protected $options = [
        '--menu'        => '게시판 구조로 이관할 워드프레스 메뉴의 slug(nicename) — 메뉴 최상위 항목이 게시판, 그 하위 항목(카테고리 대상만)이 게시판 카테고리가 된다. --dry-run 으로 목록 확인 가능 (dry-run 아니면 필수)',
    ...
    public function run(array $params)
    {
        ...
        if (! $menuNicename) {
            CLI::error('--menu 로 게시판 구조로 이관할 워드프레스 메뉴의 slug를 지정해야 합니다 (--dry-run 으로 목록 확인 가능).');
