...
> 홈페이지(`/`)를 관리자에서 편집하려면 페이지를 추가하고 슬러그를 `home`으로 입력합니다. 상태가 `공개`이고 내용이 있으면 홈페이지의 기본 샘플 섹션 대신 해당 내용이 표시됩니다. 페이지를 초안으로 바꾸거나 내용을 비우면 기존 기본 홈이 다시 표시됩니다. 상단·하단 배너와 최신 공지는 기존 관리 기능을 통해 별도로 관리합니다.
